...
func (s *ClientSuite) TestPutTemplate_SignatureVerification() {
	t := s.T()
	ctx := context.Background()

	t.Run("valid_signature_success", func(t *testing.T) {
		// Arrange: Register and authenticate user
		var login = fmt.Sprintf("test-put-template-sig-valid-%d@thingsdiary.io", time.Now().UnixMilli())
		...
		// Act: Create template with valid signature
		templateID := uuid.NewString()
		template, err := s.client.PutTemplate(ctx, diary.ID, templateID, PutTemplateParams{
			Content: "Template content",
		})

		// Assert: Template creation successful
		require.NoError(t, err)
		assert.Equal(t, templateID, template.ID)
		assert.Equal(t, "Template content", template.Content)
	})

	t.Run("missing_signature_header", func(t *testing.T) {
		// Arrange: Register and authenticate user
		var login = fmt.Sprintf("test-put-template-sig-missing-%d@thingsdiary.io", time.Now().UnixMilli())
		err := s.client.Register(ctx, login, "password-123", s.seedPhrase)
		require.NoError(t, err)
		...
		url := fmt.Sprintf("%s/api/v1/diaries/%s/templates/%s", s.client.baseURL, diary.ID, uuid.NewString())
	...
	t.Run("invalid_base64_signature", func(t *testing.T) {
		// Arrange: Register and authenticate user
		var login = fmt.Sprintf("test-put-template-sig-invalid-b64-%d@thingsdiary.io", time.Now().UnixMilli())
		err := s.client.Register(ctx, login, "password-123", s.seedPhrase)
		require.NoError(t, err)
	...
	t.Run("invalid_signature_content", func(t *testing.T) {
		// Arrange: Register and authenticate user
		var login = fmt.Sprintf("test-put-template-sig-invalid-%d@thingsdiary.io", time.Now().UnixMilli())
		err := s.client.Register(ctx, login, "password-123", s.seedPhrase)
		require.NoError(t, err)
