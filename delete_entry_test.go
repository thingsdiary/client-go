...
func (s *ClientSuite) TestEntry_DeleteEntry_ThenUpdateDeleted() {
	t := s.T()
	ctx := context.Background()

	// Arrange: Register and authenticate user
	var login = fmt.Sprintf("test-delete-entry-then-update-%d@thingsdiary.io", time.Now().UnixMilli())
	err := s.client.Register(ctx, login, "password-123", s.seedPhrase)
	require.NoError(t, err)
	...
	createdEntry, err := s.client.CreateEntry(ctx, createdDiary.ID, CreateEntryParams{
		Content: "Entry to delete and update",
	})
	require.NoError(t, err)
	require.NotNil(t, createdEntry)
	...
	// Act: Try to update deleted entry
	gotEntry, err := s.client.PutEntry(ctx, createdDiary.ID, createdEntry.ID, PutEntryParams{
		Content: "Updated Content",
	})
	require.NoError(t, err)

	// Assert
	assert.Equal(t, gotEntry.Content, "Updated Content")
}
