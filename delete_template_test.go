...
func (s *ClientSuite) TestTemplate_DeleteTemplate_ThenUpdateDeleted() {
	t := s.T()
	ctx := context.Background()

	// Arrange: Register and authenticate user
	var login = fmt.Sprintf("test-delete-template-then-update-%d@thingsdiary.io", time.Now().UnixMilli())
	err := s.client.Register(ctx, login, "password-123", s.seedPhrase)
	require.NoError(t, err)
	...
	// Create template
	createdTemplate, err := s.client.CreateTemplate(ctx, createdDiary.ID, CreateTemplateParams{
		Content: "Template to delete and update",
	})
	require.NoError(t, err)
	require.NotNil(t, createdTemplate)
	...
	// Act: Try to update deleted template
	gotTemplate, err := s.client.PutTemplate(ctx, createdDiary.ID, createdTemplate.ID, PutTemplateParams{
		Content: "Updated Content",
	})
	require.NoError(t, err)

	// Assert
	assert.Equal(t, gotTemplate.Content, "Updated Content")
}
